...
namespace CMS.Mvc
{
	public class CoditModelBinder : DefaultModelBinder
	{
		...
		internal object BindComplexModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
		{
			...
			var dictionaryType = Libs.TypeHelpers.ExtractGenericInterface(modelType, typeof(IDictionary<,>));
			...
			var enumerableType = Libs.TypeHelpers.ExtractGenericInterface(modelType, typeof(IEnumerable<>));
		...
		internal object BindSimpleModel(ControllerContext controllerContext, ModelBindingContext bindingContext, ValueProviderResult valueProviderResult)
		{
			...
			if (bindingContext.ModelType != typeof(string))
			{
				...
				var enumerableType = Libs.TypeHelpers.ExtractGenericInterface(bindingContext.ModelType, typeof(IEnumerable<>));
				if (enumerableType != null)
				{
		...
		protected override object CreateModel(ControllerContext controllerContext, ModelBindingContext bindingContext, Type modelType)
		{
			...
			if (result == null)
			{
				...
				if (isEntityCollection)
				{
					((IObjectContextAdapter)db).ObjectContext.AddObject(Libs.TypeHelpers.GetEntitySetName(typeToCreate, db), result);
				}
			}
			return result;
		}
		...
		protected override void SetProperty(ControllerContext controllerContext, ModelBindingContext bindingContext, PropertyDescriptor propertyDescriptor, object value)
		{
			...
			var isNullValueOnNonNullableType = value == null && !Libs.TypeHelpers.TypeAllowsNullValue(propertyDescriptor.PropertyType);
		...
		internal object UpdateCollection(ControllerContext controllerContext, ModelBindingContext bindingContext, Type elementType)
		{
			...
			if (isEntityCollection)
			{
				...
				{
					var name = Libs.TypeHelpers.GetPropertyValue(m, "Name", null);
					var o = srcList.FirstOrDefault(e => e.Id == ((IEntity) m).Id || (((IEntity)m).Id == 0 && (name == null || (name == Libs.TypeHelpers.GetPropertyValue(e, "Name", null)))));
					if (o == null)
					{
						sList.Add(m);
					}
