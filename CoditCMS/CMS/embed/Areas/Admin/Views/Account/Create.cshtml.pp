@using $rootnamespace$
@model $rootnamespace$.Areas.Admin.ViewModels.Accounts.CreateModel

@{
    ViewBag.CoditPageTitle = "Create";
    Layout = "~/Areas/Admin/Views/Shared/_Main.cshtml";
}

<form action="@Url.Action("Create")" id="edit-form" method="post">

    <table class="form-table">
        <tr><th>Создание нового пользователя</th></tr>
    </table>
    ...
    <table class="form-table"  id="edit-user">
        ...
        <tr>
            ...
            <td class="form-table-data">
                @Html.ListBoxFor(m=>m.UserRoles, Model.RolesSelectList)
                <div>@Html.ValidationMessageFor(m => m.UserRoles)</div>
            </td>
        </tr>
