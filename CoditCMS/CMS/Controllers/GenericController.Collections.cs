...
namespace CMS.Controllers
{
	partial class GenericController<TEntity, TDbContext>
    {
				...
				if (field is SelectSettings)
				{
					...
					else if (select.Editable && !select.Multiple)
					{
						...
						if (optionValue == null || 0 == (int)optionValue.ConvertTo(typeof(int)))
						{
							...
							var entities = ((IObjectContextAdapter)Repository.DataContext).ObjectContext.ObjectStateManager.GetObjectStateEntries((EntityState) System.Data.EntityState.Added).Where(entry => entry.Entity != null).Select(entry => entry.Entity);
		...
		protected virtual object CreateReferencePropertyElement(Type type)
		{
			...
			((IObjectContextAdapter)Repository.DataContext).ObjectContext.AddObject(TypeHelpers.GetEntitySetName(result, Repository.DataContext), result);
			return result;
		}
		...
		protected virtual void RemoveCollectionEntity(IEntity entity, IList collectionProperty, IEntity child)
		{
			...
			((IObjectContextAdapter)Repository.DataContext).ObjectContext.DeleteObject(child);
		}
